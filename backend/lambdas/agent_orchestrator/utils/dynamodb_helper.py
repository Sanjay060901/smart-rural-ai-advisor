...
# Feature flag: pagination for longer chat histories (default: OFF) — Bug 1.10
ENABLE_CHAT_PAGINATION = os.environ.get('ENABLE_CHAT_PAGINATION', 'false').lower() == 'true'

# Feature flags: medium bug fixes (default: OFF)
...
def get_chat_history(session_id, limit=40):
    ...
    """
    try:
        pagination_enabled = os.environ.get('ENABLE_CHAT_PAGINATION', 'false').lower() == 'true'
        ...
        if not pagination_enabled:
            ...
            while len(items) < limit:
                query_kwargs = {
                    'KeyConditionExpression': boto3.dynamodb.conditions.Key('session_id').eq(session_id),
                    'ScanIndexForward': False,
                    'Limit': min(40, max(1, limit - len(items)))
                }
                if last_evaluated_key:
                    query_kwargs['ExclusiveStartKey'] = last_evaluated_key
